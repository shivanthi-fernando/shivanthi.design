...
export function ButtonUnderline({
  ...
  return (
    <Link
      href={href}
      {...(external ? { target: "_blank", rel: "noopener noreferrer" } : {})}
      className={`group inline-flex w-fit items-center gap-1.5 border-b-2 border-line-strong pb-0.5 text-lg font-medium text-ink transition-colors hover:border-primary ${className}`}
    >
      {children}
